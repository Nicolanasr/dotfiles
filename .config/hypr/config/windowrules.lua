...
-- 1. Expanded Modals & File Choosers
-- Prevent web browser / IDE popups and polkit prompts from tiling
local devModals = {
    { title = "^(Confirm.*|Quit.*|Select Folder|Rename.*|Delete.*)$" },
    { title = "^(Branch.*|Commit.*|Git Credentials|Select an Image)$" },
    { class = "^(polkit-.*|org.freedesktop.impl.portal.desktop.*)$" },
    { class = "^(pinentry.*)$" }, -- GPG password prompts
}
for _, m in ipairs(devModals) do
    hl.window_rule({ match = m, float = true, center = true })
end

-- 2. Prevent Idle/Sleep During Media Playback
-- Keeps screen awake when watching full videos
hl.window_rule({
    match = { class = "^(mpv|.*vlc.*|org.kde.haruna)$" },
    idle_inhibit = "focus",
})

-- 3. Dedicated Workspace Assignments
-- Automatically send heavy apps to their own workspaces upon launch
hl.window_rule({ match = { class = "^(vesktop|discord)$" }, workspace = "3" })
hl.window_rule({ match = { class = "^(Spotify|spotify)$" }, workspace = "special:music", float = true })
...
hl.window_rule({
    ...
    float = true,
    size = { "monitor_w * 0.80", "monitor_h * 0.65" },
    center = true,
})
